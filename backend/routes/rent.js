...
router.post('/submit-rent', (req, res) => {
  const { tenantName, apartmentNo, month, rentAmount, paymentMode, status } = req.body;

  const sql = `INSERT INTO rent_payments 
    (tenant_name, apartment_no, month, rent_amount, payment_mode, status) 
    VALUES (?, ?, ?, ?, ?, ?)`;

  db.query(sql, [tenantName, apartmentNo, month, rentAmount, paymentMode, status], (err, result) => {
    if (err) {
