...
app.post('/Tenants',(req,res)=>{
  ...
  const sql = 'INSERT INTO tenants (name,flat,phone,move_in) VALUES (?, ?, ?,?)';
...
app.put('/Tenants/:id', (req, res) => {
  ...
  if (index !== -1) {
    tenants[index] = { ...req.body, id: req.params.id };
    saveTenants(tenants);
...
app.delete('/Tenants/:id', (req, res) => {
  ...
  const filtered = tenants.filter(t => t.id.toString() !== req.params.id.toString());
